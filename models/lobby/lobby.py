...
"""
Responsible for managing waiting room before game
    ...
    def run_lobby_sockets(self, socketio):
        ...
        def connect():
            ...
            if len(self.players) == 2:
                both_connected = True
            emit('message_board', {'user': session['username'], 'message': session['username'] +
                                   ' has entered the game.', 'both_connected': both_connected}, broadcast=True)
        ...
        def disconnect():
            ...
            emit('message_board', {
                 'user': session['username'], 'message': session['username'] + ' has left the room.'})
        ...
        def bird_selection(bird):
            for player in self.players:
                if player.name == session['username'] and self.check_bird_availability(bird['bird']) and not player.bird:
                    ...
                    del self.available_birds[self.available_birds.index(
                        bird['bird'])]
                    print(self.available_birds)
                    emit('selected_bird', {
                         'selected_bird': bird['bird'], 'username': session['username']}, broadcast=True)
